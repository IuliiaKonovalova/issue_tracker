...
class ProjectsView(View):
    def get(self, request):
        ...
        collab_projects = request.user.collaborated_projects.all().exclude(
...
class CreateTeamProjectView(View):
    def get(self, request):
        ...
        form.fields['collaborators'].queryset = User.objects.exclude(
            id=request.user.id
            )
        return render(
            request,
            'projects/create_project.html',
            {'form': form, 'personal': False}
            )
    ...
    def post(self, request, created_by, project_id, issue_id, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect(
                reverse(
                    'issue_detail',
                    args=[project.created_by, project.id, issue.id]
                    )
                    )
        return render(
            request,
    ...
    def post(self, request, created_by, project_id, issue_id, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect(
                reverse(
                    'issue_detail',
                    args=[issue.created_by, issue.project.id, issue.id]
                    )
                    )
        return render(
            request,
